...
name: gitbook-docs
description: >-
  Editing GitBook documentation in this Git-synced repo (RealmJoin docs).
  Use whenever creating or modifying GitBook content as markdown files —
  pages, SUMMARY.md navigation, .gitbook.yaml config, frontmatter, variables,
  and GitBook custom blocks (hints, tabs, steppers, cards, columns, etc.).
  Covers all GitBook formatting syntax and best practices needed to produce
  valid content (and pull requests) outside the GitBook web UI.
---
