...
title: RealmJoin Infrastructure
description:
keywords:
author: 
editor: lars.thiele
...
The web interface can be reached via the [realmjoin portal](https://realmjoin-web.azurewebsites.net/) and is in detailed explained in the section [RealmJoin Portal](../usage/realmjoin-portal.md). After a logging with the provided credentials, the administrator can manage the package distribution in his tenant and has access to extensive information.
...
During the package creation process, packages are signed with SHA2 (256 bit) hashes (see section [Creating packages](../packages/creating-packages.md)). Older SHA1 signed packages will still be accepted by RealmJoin. For more information on SHA2 encryption, check [the Wikipedia article on SHA2](https://en.wikipedia.org/wiki/SHA-2).
