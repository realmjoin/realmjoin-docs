...
With RealmJoin it is possible that you can manage administrators, either for local support or remote support. Of course you can create accounts as well. RealmJoin saves encrypted passwords at customers site and RealmJoin (RJ) records accesses to these passwords.
...
When a support member needs to access a secret, RJ will provide an interface to get account and password. When this happens, an update-secret command will be send to the client and the admin account will be recreated.
...
A corresponding RJ policy can trigger the creation of a persistent administrator account. The following process will be passed:

1. Starting point: Existing or new client with RealmJoin  
a) Existing client (Azure AD joined, Intune managed, RJ agend installed)  
b) New client (initialization during OOBE, Azure AD join, Intune enrollment, installation of RJ and deployed software)

2. RJ policy triggers RJ agent to create a persistent administrator account on the client.
3. RJ agent transfers the encrypted password to the RealmJoin backend.
...
2. RJ pulls the password from Azure Key Vault and displays it.
...
1. Starting point: Existing or new client with RealmJoin  
a) Existing client (Azure AD joined, Intune managed, RJ agent installed)  
b) New client (initialization during OOBE, Azure AD join, Intune enrollment, installation of RJ and deployed software)
...
4. RJ agent transfers the encrypted password to the RealmJoin backend.
...
The support staff visits the RJ WebUI again (depends on the **Configuration check interval**) and follows these steps:

1. On the device details he/she will see the name of the temporal administrator account and can request the password when clicking on the dotted text.
