...
- **Type**: Microsoft Graph
  - UserAuthenticationMethod.ReadWrite.All
    - *Lists and deletes the user's phone, Authenticator, OATH and FIDO2 methods*
  - Mail.Send *(optional — feature: Email report)*
    - *Mails the user that their MFA methods were reset when NotifyUser is enabled*
  - User.Read.All
    - *Reads the target user's display name, mail and usage location via /users/{UPN}*
