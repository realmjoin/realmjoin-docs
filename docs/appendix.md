...
The RealmJoin extensions contain CmdLets to simply register and unregister a scheduled task on the client. The task is executed by the system account. For all available CmdLets see [Scheduled Tasks](http://docs.realmjoin.com/core-extension.html#a-name-schedtasks-a-scheduled-tasks) in the Core Extension chapter.
